...
def generate_batch(param, optim_data_list, area_lookup, task_id):
    ...
    global_seed = param.get("global_seed", 42)
    task_sq = np.random.SeedSequence(global_seed, spawn_key=(task_id,))
    rng = np.random.default_rng(task_sq)
    logger.info(f"[Task {task_id}] RNG safely initialized via SeedSequence (global_seed={global_seed}, task_id={task_id})")
    ...
    for i in range(batch_size):
        ...
        batch_params.append({
            ...
            'v_offset': 0, # mean velocity clamped to zero
